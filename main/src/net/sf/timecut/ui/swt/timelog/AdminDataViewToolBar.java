 ...
 * 
 * $Id: AdminDataViewToolBar.java,v 1.10 2011/01/18 02:57:53 dragulceo Exp $
 */
package net.sf.timecut.ui.swt.timelog;

import net.sf.timecut.model.Duration;
...
import net.sf.timecut.ui.swt.SWTAdminDataView;

public class AdminDataViewToolBar extends ToolBarBase implements SelectionListener {

    private SWTAdminDataView    AdminDataView;
    private ToolItem            editButton;
    private ToolItem            addButton;
    private ToolItem            joinButton;


    public AdminDataViewToolBar(SWTAdminDataView AdminDataView) {
        super(AdminDataView.getContentArea(), AdminDataView.getMainWindow().getIconSet(), 1);
    ...
    private ToolItem createEditButton() {
        ...
        item.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent evt) {
                AdminDataViewToolBar.this.AdminDataView.editSelection();
            }
        });
        return item;
    }
    ...
    private ToolItem createAddButton() {
        ...
        item.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent evt) {
                AdminDataViewToolBar.this.AdminDataView.addNewEntry();
            }
        });
        return item;
    }
    ...
    private ToolItem createDeleteButton() {
        ...
        item.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent evt) {
                AdminDataViewToolBar.this.AdminDataView.removeSelected();
            }
        });
        return item;
    }
    ...
    private ToolItem createJoinButton() {
        ...
        item.addSelectionListener(new SelectionAdapter() {
            public void widgetSelected(SelectionEvent evt) {
                AdminDataViewToolBar.this.AdminDataView.joinSelected();
            }
        });
    ...
    public void widgetSelected(SelectionEvent evt) {
        ...
        if (source != null && source instanceof Table) {
            ...
            if (items.length > 0) {
                ...
                AdminDataView.getMainWindow().getStatusLine().setSelectionLabel("Duration: " + (new Duration(total_duration)).toString());
                if (items.length > 1) {
