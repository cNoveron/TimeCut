...
public class SWTAdminTotalsTableView {
    ...
    public SWTAdminTotalsTableView(SWTWindow Window) {
        _Window = Window;
    ...
    public void updateTable() {
        ...
        if (_Window.getTreeView().getTree().getSelectionCount() > 0) {
                ...
                if (selectedItem != null) {
                        _timeLog = DataManager.getInstance().getWorkspace()
                                        .getTimeLog();
    ...
    private void addRecords(Task task) {
        ...
        if (DataManager.getInstance().getFilter() != null) {
            filter = (TimeRecordFilter) DataManager.getInstance().getFilter()
                .clone();
        }
        else {
	...
	private void addTableItem(TotalsCalculator data, boolean highlight) {
		...
		TableItem item = new TableItem(_table, SWT.NONE);
        Totals totals = data.getTotals(_timeLog, DataManager.getInstance().getFilter());
		if (highlight) {
        ...
        for (int i = 0; i < items.length; i++) {
            ...
            if (dataObject instanceof TotalsCalculator && dataObject instanceof ProjectTreeItem) {
                ...
                buf.append(totalsCalculator.getTotals(_timeLog,
                        DataManager.getInstance().getFilter()).getDuration().toString());
                if (i < items.length - 1) {
                    buf.append("\n");
                }
            }
